...
/** Animated banner with ASCII logo */
...
export function summaryBox(title: string, items: { label: string; value: string; highlight?: boolean }[]): void {
  ...
  const maxVal = Math.max(...items.map(i => i.value.length));
  const boxWidth = Math.min(width - 4, maxLabel + maxVal + 6);
  ...
  for (const item of items) {
    ...
    const value = item.highlight ? c.cyan(c.bold(item.value)) : item.value;
    console.log(c.cyan(c.bold('  \u2502')) + ` ${label}  ${value}`.padEnd(boxWidth) + c.cyan(c.bold('\u2502')));
  }
  console.log(c.cyan(c.bold('  \u2514' + '\u2501'.repeat(boxWidth) + '\u2518')));
  console.log();
}
